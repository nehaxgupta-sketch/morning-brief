  ...
  if (stage === 'sent') {
    return (
      <div style={{ minHeight: '100vh', background: '#1A1A1A', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '32px 24px', textAlign: 'center' }}>
        ...
        <h1 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: '26px', fontWeight: '700', color: '#F5F1EA', marginBottom: '16px' }}>Reset link sent</h1>
        ...
        <Link href="/login" style={{ fontFamily: "'DM Mono', monospace", fontSize: '10px', letterSpacing: '1px', color: '#C8A45A', textDecoration: 'underline' }}>Back to sign in</Link>
      </div>
    )
  }
    ...
    <div style={{ minHeight: '100vh', background: '#1A1A1A', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ width: '100%', maxWidth: '360px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '40px' }}>
          ...
          <h1 style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '18px', fontWeight: '400', color: '#B0A898', marginBottom: '8px' }}>Reset your password</h1>
          <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '13px', color: '#666', lineHeight: '1.5' }}>Enter your email and we'll send you a reset link.</p>
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div>
            <label style={{ display: 'block', fontFamily: "'DM Mono', monospace", fontSize: '9px', letterSpacing: '2px', color: '#999', marginBottom: '8px', textTransform: 'uppercase' as const }}>Email address</label>
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="priya@email.com" onKeyDown={e => e.key === 'Enter' && handleReset()}
          ...
          <button onClick={handleReset} disabled={loading} style={{ width: '100%', padding: '16px', background: loading ? '#333' : '#C8A45A', color: loading ? '#666' : '#1A1A1A', fontFamily: "'DM Sans', sans-serif", fontSize: '14px', fontWeight: '600', letterSpacing: '1px', textTransform: 'uppercase', border: 'none', cursor: loading ? 'not-allowed' : 'pointer', borderRadius: '2px', minHeight: '52px' }}>
            {loading ? 'Sending...' : 'Send Reset Link →'}
          </button>
        </div>

        <div style={{ textAlign: 'center', marginTop: '24px', fontFamily: "'DM Sans', sans-serif", fontSize: '13px' }}>
