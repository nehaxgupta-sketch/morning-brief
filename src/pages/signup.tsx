...
export default function Signup() {
  ...
  const inputStyle = (filled: boolean) => ({ width: '100%', padding: '14px 16px', background: '#2A2A2A', border: `1px solid ${filled ? '#C8A45A' : '#333'}`, color: '#F5F1EA', fontFamily: "'DM Sans', sans-serif", fontSize: '15px', outline: 'none', borderRadius: '2px' })
  const labelStyle = { display: 'block', fontFamily: "'DM Mono', monospace", fontSize: '9px', letterSpacing: '2px', color: '#999', marginBottom: '8px', textTransform: 'uppercase' as const }
  ...
  if (stage === 'verify') {
    return (
      <div style={{ minHeight: '100vh', background: '#1A1A1A', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '32px 24px', textAlign: 'center' }}>
        ...
        <button type="button" onClick={async () => { await supabase.auth.resend({ type: 'signup', email }); setError('Resent! Check your inbox.') }} style={{ background: 'none', border: 'none', fontFamily: "'DM Mono', monospace", fontSize: '10px', letterSpacing: '1px', color: '#C8A45A', cursor: 'pointer', textDecoration: 'underline', minHeight: '44px' }}>Resend verification email</button>
